...
API Key 使用 IntelliJ Platform 的 **Password Safe** 保存，插件的普通配置文件不保存明文 API Key。Key 会按完整 API 地址区分，因此切换服务地址后可能需要重新设置对应 Key。

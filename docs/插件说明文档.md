...
在“模型名称”下方可以单独选择 **AI 输出语言**。该语言只控制模型生成的提交信息，不会随提交模板语言实时变化；首次创建配置或旧配置缺少该字段时，默认采用当前提交内容语言。点击 **Apply** 或 **OK** 后，选择结果会持久化到插件的 AI 配置中，后续可独立调整为中文、英文、日文等明确语言。
...
- **生成成功**：直接保留最终提交信息，不再弹出成功气泡通知。

About 页面中的复制操作只在按钮文字中反馈成功或失败，按钮会暂时禁用，约 2 秒后恢复为“复制”。
...
AI 模型页的非敏感配置由 `AiPreferencesState` 持久化到 `commit-template-ai.xml`；API Key 仍只由 Password Safe 管理。`generationLanguage` 在生成开始时复制到 `AiGenerationConfigSnapshot`，保证一次请求期间配置不会被中途修改影响。

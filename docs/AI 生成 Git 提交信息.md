...
- 不自动写入或自动确认提交；
- 用户手动点击“应用到提交信息”后才将格式化结果写入 IDEA 的 Commit Message 输入框；
...
该项默认勾选，且只控制每次生成前是否展示实际外发的 Diff，不影响首次代码信息传输授权。

- **勾选（默认）**：构建一次 `AiGenerationContext` 后，展示发送摘要、完整筛选 Diff、截断状态和排除原因；用户点击 `确认并生成` 后才联网；
- **未勾选**：完成本地筛选并确认存在可发送 Diff 后，直接发起 AI 请求，不展示 Diff 确认窗口；
- 首次代码信息传输授权始终不可跳过；用户拒绝授权仍会关闭 AI 功能。

发送前检查窗口应显示提示：

```text
已启用“发送前检查将传输的代码差异”。如不需要每次检查，可前往 AI 模型设置关闭此选项。
```

其中“前往 AI 模型设置”为可点击链接或按钮，直接打开插件的 `AI 模型` 设置页。跳转仅关闭当前检查窗口，不构建新的请求，也不自动发送 Diff。
...
flowchart TD
    ...
    M -->|否| N[显示不可应用原因]
    M -->|是| O[用户点击应用到提交信息]
    O --> P[本地格式化后写入 Commit Message 输入框]
```
...
1. 解析 JSON；
2. 校验 `type` 位于当前允许类型中；
3. 不执行本地自然语言识别或阻断；提交内容语言由系统提示词强制约束，并由用户在应用前审阅确认。这样不会误拦截包含代码标识符、产品名或英文技术术语的正常提交信息；
4. 使用既有 `CommitMessageValidator` 校验标题、scope、body、breaking change 和 issue；
5. 使用既有 `CommitMessageFormatter` 生成最终提交文本；
6. 仅在用户点击“应用到提交信息”后，使用既有 `CommitMessageFormatter` 格式化并写入 IDEA 的 Commit Message 输入框；不回填或修改模板弹窗字段。

## 9. 实施步骤

1. 在 `AiPreferencesState` 增加全局 `AiDataTransferConsent` 状态；保持 API Key 仍仅存于 Password Safe；
2. 新增默认值为 `true` 的全局配置 `checkDiffBeforeSending`，在 AI 模型设置页提供开关；
3. 新增独立的 `AiDataTransferConsentDialog`，所有 AI 入口共用；拒绝时将 `enabled` 设为 `false` 并持久化 `DECLINED`；用户从设置页重新启用 AI 时，重置授权状态为 `UNDECIDED`；
4. 在 `AiQuickGenerateAction` 和模板弹窗入口中，先经过授权守卫，再采集任何变更；两个入口均使用当前 Commit Message 输入框作为唯一回填目标；
5. 新增或重构 `AiGenerationContext`，替代散落的 `request + diff + summary` 临时变量；
6. 将 `AiIncludedChangesCollector` 改为一次返回 `DiffCollectionResult`，不再保留 metadata-only 请求入口；
7. 新增 `AiCommitTemplateContextRenderer`，把 `EffectiveCommitTemplateSettings` 转为稳定的文本上下文；
8. 重构 `AiPromptRenderer`：用户消息固定为“模板上下文 + 已筛选 Diff”；系统提示词负责强制提交内容语言与当前 Git 模板语言一致；
9. 精简 `AiGenerationDialog`：移除 Diff 勾选框和元数据分支；根据 `checkDiffBeforeSending` 决定显示确认窗口或直接请求；确认窗口提供跳转 AI 模型设置页的快捷入口；
10. 保留 `OpenAiCompatibleProvider` 的 OpenAI Compatible SSE 实现、取消能力和脱敏原则；
11. 保留 `AiSuggestionParser`、`AiSuggestionValidator` 和提交信息回填；删除模板弹窗字段回填分支；不增加本地自然语言识别拦截；
12. 删除正式代码中的 `DEVELOPMENT_DIFF_PREVIEW` 和相关 i18n 文案；开发请求检查仅保留 Internal Mode 的只读折叠面板；
13. 为授权接受/拒绝、重新授权、发送前检查开关、context 构建、Diff 过滤、提示词渲染和本地校验补充单元测试；
14. 在 IDE Sandbox 手工验证：首次同意、首次拒绝、重新开启后的重新授权、检查开关开启/关闭、跳转设置、中文/英文等不同 Git 模板语言、空变更、敏感文件、截断 Diff、SSE 取消、非法 JSON、工具栏入口写入 Commit Message、模板弹窗入口写入 Commit Message。

## 10. 对现有实现的取舍

保留：Password Safe、Included Changes 获取、敏感 Diff 过滤、OpenAI Compatible SSE、流式输出、本地 JSON 校验、本地 formatter、唯一的 Commit Message 回填目标。

删除或合并：`METADATA` 传输模式、Diff 发送复选框、请求时重复构建 Diff、正式发布中的开发预览逻辑、模板弹窗字段回填逻辑，以及仅为两条回填路径存在的 UI/提示词分支。
